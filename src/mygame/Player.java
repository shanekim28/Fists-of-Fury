...
public class Player extends AbstractAppState implements PhysicsCollisionListener {
	...
	public void Die() {
		if (lives > 0) {
			...
			rb.setMass(10);
			client.send(new UpdatePlayerMassMessage(10, client.getId()));
		}
	}

	public void TakeDamage(float force) {
		if (force * (15 - GetDefense()) * 0.0001f * rb.getMass() > 1) {
			damagePercent += rb.getMass() / (force * (15 - GetDefense()) * 0.0001f * rb.getMass());
			rb.setMass(rb.getMass() / (force * (15 - GetDefense()) * 0.0001f * rb.getMass()));
			client.send(new NetworkUtility.UpdatePlayerMassMessage(rb.getMass() / (force * (15 - GetDefense()) * 0.0001f * rb.getMass()), client.getId()));
		}
	}
	...
	public void collision(PhysicsCollisionEvent event) {
		//if (Player.class.isInstance(event.getNodeA()) && Player.class.isInstance(event.getNodeB()))
